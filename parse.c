...
int parse_commandline(int argc, char *argv[])
{
  ...
  if (options.opt_ants)
  {
    total_ants = atol(options.arg_ants);
    fputs("  -m  --ants ", stdout);
    if (options.arg_ants != NULL)
      printf("with argument \"%ld\"\n", total_ants);
    check_out_of_range(total_ants, 1, MAX_ANTS - 1, "n_ants");
  }
  else
  {
    if (total_ants < 0)
  ...
  padded_ants = (total_ants - 1) / procs + 1;
  long int st = padded_ants * rank;
  long int ed = (padded_ants * (rank + 1)) > total_ants ? total_ants : (padded_ants * (rank + 1));
